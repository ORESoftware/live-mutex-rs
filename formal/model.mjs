...
function checkState(s) {
  ...
  if (s.holder === null) {
    fail(s.fence === 0 && s.expiresAt === 0, "free lock has no grant metadata", s);
  } else {
...
const actions = () => {
  ...
  for (const client of CLIENTS) {
    ...
    for (const fence of [1,2,3,4]) {
      out.push({kind:"release", client, fence}, {kind:"renew", client, fence});
    }
  }
  return out;
};
...
function explore() {
  ...
  console.log(JSON.stringify({
    ...
    invariants:[
      ...
      "stale-release-side-effect-free",
      "ttl-expiry-progress"
    ]
  }));
}
...
function replay(doc) {
  ...
  for (const action of doc.actions ?? []) {
    const tr = step(state, action);
    outcomes.push({accepted:tr.accepted, reason:tr.reason});
    state = tr.next;
  }
