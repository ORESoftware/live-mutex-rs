...
compacted prefix, but it does not yet transfer arbitrary live broker state. Lock
UUID replay is deterministic across nodes; fencing tokens are still generated by
each broker instance, so performance comparisons should ignore fencing tokens
until the committed log also carries deterministic fencing-token results.
