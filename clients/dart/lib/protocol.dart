...
class LockRequest extends Request {
  ...
  @override Map<String, dynamic> toJson() {
    final m = <String, dynamic>{
      'type': type, 'uuid': uuid,
      'force': force, 'retryCount': retryCount,
      'keepLocksAfterDeath': keepLocksAfterDeath, 'ttl': ttl,
    };
    ...
    switch (t) {
      ...
      case 'compositeLock':
        return CompositeLockResponse(
          ...
          acquired: j['acquired'] as bool? ?? false,
          lockUuid: j['lockUuid'] as String?,
          fencingTokens: (j['fencingTokens'] as Map?)?.map((k, v) => MapEntry(k as String, (v as num).toInt())),
          error: j['error'] as String?,
        );
      case 'unlock':
      ...
      case 'endReadResult':
        return EndReadResultResponse(uuid: u, key: j['key'] as String? ?? '', readersCount: (j['readersCount'] as num?)?.toInt() ?? 0);
      case 'endWriteResult':
        return EndWriteResultResponse(uuid: u, key: j['key'] as String? ?? '', readersCount: (j['readersCount'] as num?)?.toInt() ?? 0, writerFlag: j['writerFlag'] as bool? ?? false);
      case 'lockInfo':
...
class LockResponse extends Response {
  const LockResponse({
    required super.uuid, required this.key, required this.acquired,
    required this.lockRequestCount, this.lockUuid, this.fencingToken,
    this.readersCount, this.error,
  });
  final String key;
  final bool acquired;
...
class CompositeLockResponse extends Response {
  const CompositeLockResponse({
    required super.uuid, required this.keys, required this.acquired,
    this.lockUuid, this.fencingTokens, this.error,
  });
  final List<String> keys;
  final bool acquired;
  final String? lockUuid;
...
class LockInfoResponse extends Response {
  const LockInfoResponse({
    required super.uuid, required this.key, required this.isLocked,
    required this.lockholderUuids, required this.lockRequestCount,
    required this.readersCount, required this.writerFlag,
  });
  final String key;
  final bool isLocked;
