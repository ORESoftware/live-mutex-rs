...
//
// The broker speaks flat-ish JSON objects (strings, numbers, bools, null,
// arrays of strings, and string->number maps), so this is a compact
// recursive-descent parser + serializer rather than a general library. We
// keep numbers as their original text and parse on demand so 64-bit fencing
// tokens never lose precision through a double.
#pragma once
...
class Value {
 public:
  ...
  // Object helpers (return defaults / nullptr when the key is absent).
      ...
      if (c == '\\') {
        ...
        switch (e) {
          ...
          case 'u': {
            ...
            i_ += 4;
            // Encode the BMP code point as UTF-8 (sufficient for keys/uuids).
            if (code < 0x80) {
