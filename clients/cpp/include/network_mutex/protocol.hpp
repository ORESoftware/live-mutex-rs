// Wire protocol mirror of src/protocol.rs with fail-closed fencing admission.
#pragma once
...
constexpr uint64_t kMaxFencingToken = 9007199254740991ULL;
constexpr const char* kProtocolVersion = "0.1.0";

enum class RequestType { Version, Auth, Lock, Unlock, RegisterRead, RegisterWrite, EndRead, EndWrite, LockInfo, Ls, Heartbeat };
enum class ResponseType { Version, Auth, Lock, CompositeLock, Unlock, RegisterReadResult, RegisterWriteResult, EndReadResult, EndWriteResult, LockInfo, LsResult, Reelection, Error, Ok, Unknown };
...
inline std::string frame(const json::Object& obj) { return json::Value(obj).dump() + "\n"; }
inline std::string version_request(const std::string& uuid, const std::string& value = kProtocolVersion) { return frame({{"type", to_wire(RequestType::Version)}, {"uuid", uuid}, {"value", value}}); }
