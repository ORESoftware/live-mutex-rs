%%% TCP transport FFI for the Gleam client. We deliberately use plain
%%% gen_tcp in {packet, line} mode so each newline-delimited JSON frame
%%% comes back as a single message — the Gleam side then does pattern
%%% matching on the parsed Response variant.

-module(dd_rust_network_mutex_client_ffi).
